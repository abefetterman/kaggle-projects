...
def train_s2(fold=1, disable_progress=False):
    directory = './data/'
    ...
    #optimizer = Hybrid(model.parameters(), lr=learning_rate, etad=0.8, etam=0.8, weight_decay=1e-4)
    ...
    for e in range(epoch):
        train_loss = []
        for sample in tqdm(data.DataLoader(dataset, batch_size = 12, shuffle = True), disable=disable_progress):
        ...
        if avg_iou > best_iou:
            ...
            save_checkpoint(f'unet-fold{fold}-s2-best.pth', model, optimizer)
            best_iou = avg_iou
